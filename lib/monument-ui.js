...
/* harmony import */ var _perspective__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./perspective */ "./src/perspective.js");
...
var Element = styled_components__WEBPACK_IMPORTED_MODULE_0___default.a.div(_templateObject(), function (_ref) {
  ...
  return "\n    transition: all 300ms ease;\n\n    &:hover {\n      cursor: pointer;\n    }\n\n    &:not(:focus):not(:active) {\n      box-shadow: ".concat(Object(_perspective__WEBPACK_IMPORTED_MODULE_1__["count"])(perspective.x, perspective.y, color), ";\n    }\n\n    ").concat(hoverable && "\n      &:not(:active):hover {\n        box-shadow: ".concat(Object(_perspective__WEBPACK_IMPORTED_MODULE_1__["count"])(perspective.x > 0 ? perspective.x + 2 : perspective.x - 2, perspective.y > 0 ? perspective.y + 2 : perspective.y - 2, color), ";\n        transform: translate(").concat(perspective.x > 0 ? -2 : 2, "px, ").concat(perspective.y > 0 ? -2 : 2, "px);\n      }\n    "), "\n\n    ").concat(focusable && "\n      &:focus,\n      &:active {\n        box-shadow: 0, 0, 0 rgba(0, 0, 0, 0);\n        transform: translate(".concat(perspective.x, "px, ").concat(perspective.y, "px);\n      }\n    "), "\n  ");
});
/*{
...
var count = function count(x, y, color) {
  ...
  var newValue = [];
  var j = Math.abs(vertical) / Math.abs(horizontal);
  var k = 0;

  for (var i = 1; i <= Math.abs(horizontal); i++) {
    k += j;
    var horiz = Math.abs(x) > Math.abs(y) ? i : k;
    var vert = Math.abs(x) < Math.abs(y) ? i : k;
    newValue = [].concat(_toConsumableArray(newValue), ["".concat(x > 0 ? horiz : -horiz, "px ").concat(y > 0 ? vert : -vert, "px 0 ").concat(color)]);
  }
